 ...
 * • We try both classpath resources and filesystem paths so it works from IDE
 * or build.
 *
 * @author Kirtan Patel
 * @author Rudra Patel
...
public class GameDataLoader {
    ...
    // Candidate locations for playerData.json (same pattern we already use for
    // game.json)
    ...
    public ArrayList<User> getUsers() {
        ...
        if (arr == null)
            return out; // no users section -- empty list
        ...
        for (Object o : arr) {
            ...
            String email = uo.get("email") == null ? null : uo.get("email").toString();

            // Our User constructor (UUID, username, password, email)
            ...
            if (invObj != null) {
                ...
                if (capacity <= 0)
                    capacity = (items == null) ? 26 : Math.max(26, items.size());
                Inventory inv = new Inventory(capacity);
                if (items != null) {
                    for (Object it : items) {
                        if (it != null)
                            inv.addItem(it.toString());
                    }
                }
                u.setInventory(inv);
            }
     ...
     *
     * <p>
     * Looks up the room by its {@code roomID}. If not found and the id is in the
     * form {@code "roomN"} (e.g., "room1"), it falls back to the N-th room
     * (1-based). If the room or its {@code puzzles[]} is missing, returns an empty
     * list.
     * </p>
     */
    public java.util.List<Puzzle> loadPuzzlesForRoom(String roomId,
        ...
        for (Object rObj : rooms) {
            ...
            if (roomId != null && roomId.equals(id)) {
                target = ro;
                break;
            }
        }
        if (target == null && roomId != null && roomId.startsWith("room")) {
            try {
                ...
                if (idx >= 0 && idx < rooms.size())
                    target = (org.json.simple.JSONObject) rooms.get(idx);
        ...
        for (Object pObj : puzzles) {
            ...
            if (textOut != null && id != null) {
                if (!prompt.isBlank())
                    textOut.put(id + ":prompt", prompt);
                if (!hint.isBlank())
                    textOut.put(id + ":hint", hint);
                if (!reward.isBlank())
                    textOut.put(id + ":reward", reward);
            }
        }
    ...
    public ArrayList<Rooms> getRooms() {
        ...
        for (Object roomObj : roomsArray) {
            if (!(roomObj instanceof JSONObject))
                continue;
     ...
     * 3) If not available, try addEntry(username, difficulty, timeSeconds, score,
     * date).
     */
    public Leaderboard getLeaderboard() {
        ...
        if (!ok)
            setIfPresent(lb, "setLB", List.class, list);
        return lb;
    }
     ...
     * If the input is null, a Number, or a String that can be parsed to an int,
     ...
     * Returns true when successfully invoked, false if the method is absent or
     * fails.
     */
     ...
     * Invokes a getter method with the given name and returns the result as a
     * string.
     ...
     *
     * The main method demonstrates how to use the GameDataLoader class to load data
     * from the JSON files and print it to the console.
     */
    ...
    public static class TextPuzzle extends Puzzle {
        ...
        /**
         * Retrieves the hint associated with this TextPuzzle, or the puzzle's global
         * hint if no specific hint is set.
         * 
         * @return the hint string associated with this TextPuzzle, or the global hint
         *         if no specific hint is set.
         */
        @Override
         ...
         * 
         * @return the letter rewarded after solving the puzzle, or null if no reward is
         *         set.
         */
        @Override

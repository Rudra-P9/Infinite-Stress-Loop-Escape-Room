...
public class DriverScenario {
    ...
    /**
     * Driver scenario for testing the entire Escape Room application from start to
     * finish.
     * 
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (accounts.getUser(bro) == null) {
            accounts.createAccount(bro, "loganPass123", "dummy@example.com");
        ...
        if (accounts.getUser("Logan Rivers") == null) {
            accounts.createAccount("Logan Rivers", "leniTry", "leni@example.com");
            writer.saveAccounts(accounts);
        ...
        if (accounts.getUser(lena) == null) {
            accounts.createAccount(lena, "leniSecret", "lena@example.com");
            writer.saveAccounts(accounts);
        ...
        try {
            ...
            for (int k = 0; k < Math.min(sp, order.length); k++) {
                ...
                for (com.escape.model.Rooms r : all) {
                    if (rid.equalsIgnoreCase(r.getRoomID())) {
                        rr = r;
                        break;
                    }
                }
        ...
        long finalScore = 0;
        try {
            finalScore = facade.calculateFinalScore();
        } catch (Throwable ignored) {
        }
        try {
     ...
     * The certificate is a text file containing the user's name, game title, date,
     * difficulty, hints used, and final score.
